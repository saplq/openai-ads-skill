...
1. **Authenticate** — first look for `ads-manager-api-key.txt` beside `SKILL.md`. If no profile exists, any authenticated command automatically validates it with `GET /ad_account`, stores it securely outside the repository, and removes the dropped file. Fall back to hidden `auth login` or `auth import-file --file PATH`; never ask for a key in chat or argv.
